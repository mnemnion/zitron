...
```sh
zig fetch --save "https://github.com/mnemnion/zitron/archive/refs/tags/v0.2.7.tar.gz"
```
...
Tip: It can be useful to have tokens which are not actually used in the
grammar: you may wish to return a `WHITESPACE` or a `COMMENT` from the
lexer, without the obligation of introducing those to the parser.  Naming
them using `%token` will accomplish this.
